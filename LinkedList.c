...
Node* create_node(int ind, int data) {
    ...
    newNode->indice=ind ; /* Impostiamo l'indice dell'elemento */
...
void insert_at_head(Node **head, Node *node_to_insert) {
    ...
    *head = node_to_insert;       // La nostra "testa" della lista ora e' il nuovo nodo
}
...
int main() {
    // Inizialmente, la nostra lista e' vuota
    Node *head = NULL;
    ...
    // Inseriamoli in testa. L'ordine finale sara' 40 -> 30 -> 20 -> 10
